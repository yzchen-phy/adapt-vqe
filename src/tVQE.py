    ...
    def hessian(self,parameters):
        ...
        for j in reversed(range(0,len(parameters))):
            ...
            for i in reversed(range(0,j+1)):
                #print("Term 1")
                #print(bra_sigma_j.dot(self.G[j]).dot(op_j_d_i).dot(self.G[i]).dot(ket_i).toarray()[0][0].real) 
                #print("Term 2")
                #print(bra_j.dot(self.G[j]).dot(op_j_h_i).dot(self.G[j]).dot(ket_i).toarray()[0][0].real)
